...
const AdminPage = () => {
  ...
  const mutation = useMutation(({ id, isActive }: { id: string, isActive: boolean }) => {
  ...
  return (
    <MainLayout title="Записи" isArrow>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>{appointments?.sort((a: Appointment, b: Appointment) => new Date(a.dateTime).getTime() - new Date(b.dateTime).getTime()).map((item: Appointment, index: number) => (
        <div key={index} className={styles.appointment}>

          <div style={{ padding: 4, display: 'flex', justifyContent: 'space-between' }}>
            <div>
              <p>{item.name}</p>
              <a className={styles.phone} href={`tel:${item.phone}`}>{item.phone}</a>
            </div>

            <div>
              {item.cards?.map((card, indx) => (
                <p style={{ fontSize: 16, maxWidth: 150, textAlign: 'center' }} key={indx}>{card}</p>
              ))}
            </div>
          </div>
