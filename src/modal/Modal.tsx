...
const Modal = ({ active, setActive, setState, state }: TProps) => {
    ...
    const handlers = {
        ...
        handlePhoneChange: (event: ChangeEvent<HTMLInputElement>) => {
            ...
            setState(prevState => ({ ...prevState, phone: Number(sanitizedValue) }));
    ...
    return (
        <div className={active ? `${styles.modal} ${styles.active}` : `${styles.modal}`} onClick={() => { setActive(false); setState(prevData => ({ ...prevData, phone: +7, name: '' })) }}>
            <div className={styles.modal__content} onClick={e => e.stopPropagation()}>
                {isError ? <div>
                    <h3 style={{ color: 'red', textAlign: 'center' }}>Выбранное время уже  занято!</h3> <br />
                </div> : <>
                ...
                <Link style={{ textDecoration: 'none' }} to={isError ? '/date' : '/'}>
                    <Button disabled={isDisabled} onClick={isError ? handlers.closeModal : actions.onSubmit} title={isError ? "Выбрать другое время" : "Записаться"} />
                </Link>
